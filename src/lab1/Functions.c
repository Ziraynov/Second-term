...
{
    char* buf = strstr(html_string, "class='result__attr_var  cr-result__attr_odd'>Фактическая диагональ</td><td class='result__attr_val");
    while (buf != NULL)
    {
        buf = strstr(strstr(buf, "class='result__attr_var '>Разрешение</td><td class='result__attr_val"), "class='result__attr_var  cr-result__attr_odd'>Фактическая диагональ</td><td class='result__attr_val");
        (*number)++;
    }
}
monitor* parse_html(char* path, int* number) {
    ...
    for (int i = 0; i < *number; i++)
    {
        ...
        arr[i].matrix = get_the_word(&html_string, "</td></tr><tr><td class='result__attr_var  cr-result__attr_odd'>Тип матрицы</td><td class='result__attr_val  cr-result__attr_odd'>", "</td></tr><tr><td class='result__attr_var '>");
...
void sort(monitor* arr, int len)
{
    ...
    case 1: qsort(arr, len, sizeof(monitor), (int (*)(const void*, const void*)) comp_name); break;
    case 2:   qsort(arr, len, sizeof(monitor), (int (*)(const void*, const void*)) comp_price); break;
    case 3:    qsort(arr, len, sizeof(monitor), (int (*)(const void*, const void*)) comp_producer); break;
    case 4:   qsort(arr, len, sizeof(monitor), (int (*)(const void*, const void*)) comp_name_price); break;
    case 5:  qsort(arr, len, sizeof(monitor), (int (*)(const void*, const void*)) comp_producer_matrix); break;
    }
